...
def show_messages (text) :
  messages_str = [
  f'[_['role '1}: {_['content '1}.' for _ in st.session_state ["messages"][1:]
  ]
...
if st.button ("Send"):
  with st.spinner ("Generating response..."):
    st. session_state["messages"] += [f"role": "user", "content": prompt}]
    response = openai. ChatCompletion.create
      model="gpt-3.5-turbo", messages=st. session_state ["messages"]
    )
    message_response = response ["choices" ][o]['message"]["content"]
